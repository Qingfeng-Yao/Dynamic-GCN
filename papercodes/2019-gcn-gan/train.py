...
for epoch in range(config['pretrain_epoches']):
    for i, data in enumerate(train_loader):
        ...
        in_shots, out_shot = data  # torch.Size([64, 10, 38, 38]) torch.Size([64, 38, 38])
        in_shots, out_shot = in_shots.cpu(), out_shot.cpu()
        predicted_shot = generator(in_shots)
        out_shot = out_shot.view(config['batch_size'], -1)
        loss = mse(predicted_shot, out_shot)# UserWarning: Using a target size (torch.Size([64, 1444])) that is different to the input size (torch.Size([1, 64, 1444]))
        loss.backward()
...
for epoch in range(config['gan_epoches']):
    for i, (data, sample) in enumerate(zip(train_loader, sample_loader)):
        ...
        out_shot = out_shot.view(config['batch_size'], -1)
        mse_loss = mse(predicted_shot, out_shot)#UserWarning: Using a target size (torch.Size([64, 1444])) that is different to the input size (torch.Size([1, 64, 1444]))
